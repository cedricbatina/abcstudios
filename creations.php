          ...
          <div class="row row-cols-1 row-cols-md-3 g-4 mb-3 mt-3">
            <?php foreach ($result as $row) : ?>
              <a href="creation.php?id=<?= $row['id'] ?>">
                <div class="card ">
                  <img src="<?= $row['image'] ?>" class="card-img-top img-thumbnail card-img" alt="creation">
                  <div class=" card-body">
                    <h5 class="card-title"><?php echo nl2br(htmlspecialchars($row['title'])) ?></h5>
                    <p class="card-text">
                    <p class="creation_date"><?= date("jS M, Y", strtotime($row['date'])) ?></p>
                    <p class="card-text"><strong><?php echo nl2br(htmlspecialchars(substr($row['post'], 0, 200))) ?></strong></p>
