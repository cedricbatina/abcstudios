...
if ($_SERVER['REQUEST_METHOD'] == "POST" && !empty($_POST['action']) && $_POST['action'] == 'post_delete') {
	...
	$image_added = false;
	if (!empty($_FILES['image']['name']) && $_FILES['image']['error'] == 0 && ($_FILES['image']['type'] == "image/jpeg" || $_FILES['image']['type'] == "image/jpg" || $_FILES['image']['type'] == "image/png")) {
		//file was uploaded
		$folder = "uploads/";
	...
	if ($image_added == true) {
		$query = "update posts set post = '$post', title = '$title', image = '$image' where id = '$id' && user_id = '$user_id' limit 1";
	} else {
		$query = "update posts set post = '$post', title = '$title' where id = '$id' && user_id = '$user_id' limit 1";
	}
	...
	$email = addslashes($con, $_POST['email']);
	$password = addslashes($con, $_POST['password']);
	$id = $_SESSION['info']['id'];
	...
	//adding post
	$image = "";
	if (!empty($_FILES['image']['name']) && $_FILES['image']['error'] == 0 && $_FILES['image']['type'] == "image/jpeg" || $_FILES['image']['type'] == "image/png" || $_FILES['image']['type'] == "image/jpg" || $_FILES['image']['type'] == "image/pdf") {
		//file was uploaded
		$folder = "uploads/";
		if (!file_exists($folder)) {
			mkdir($folder, 0777, true);
	...
	$query = "insert into posts (title,post,image,user_id,date) values ('$title','$post','$image','$user_id','$date')";
	...
	<div style="margin: auto;max-width: 600px">
		...
		<?php else : ?>
			...
			<div style="margin: auto;max-width: 600px;text-align: center;">
				...
				<a href="myprofile.php?action=edit">
					<button>Edit profile</button>
				</a>

				<a href="myprofile.php?action=delete">
			...
			<form method="post" enctype="multipart/form-data" style="margin: auto;padding:10px;">
				...
				image: <input type="file" name="image"><br>
				<input name="title" type="text" placeholder="Titre" rows="1"><br>
				<textarea name="post" placeholder="Texte" rows="8"></textarea><br>
						...
						<div style="background-color:white;display: flex;border:solid thin #aaa;border-radius: 10px;margin-bottom: 10px;margin-top: 10px;">
							...
							<div style="flex:8">
								...
								<div>
									...
									<a href="myprofile.php?action=post_edit&id=<?= $row['id'] ?>">
										<button>Edit</button>
									</a>

									<a href="myprofile.php?action=post_delete&id=<?= $row['id'] ?>">
										<button>Delete</button>
									</a>
									<br><br>
								</div>
